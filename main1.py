...
# Register the setid command handler
@client.on(events.NewMessage(pattern='/setid (.*)', from_users=main_ids))
async def handle_setid(event):
    global temp_ids
    new_temp_ids = event.pattern_match.group(1).split()
    
    # Save new_temp_ids to ids.txt
    with open('ids.txt', 'w') as file:
        file.write(' '.join(new_temp_ids))
    await event.respond('<code>✅ Temporary IDs set successfully!</code>', parse_mode='html')
